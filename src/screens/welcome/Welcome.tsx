...
export default function App() {
  ...
  const simulateMovement = () => {
    setCurrentLocation((prevLocation) => {
      if (prevLocation) {
        const newLatitude = prevLocation.coords.latitude + 0.0001; // Increment latitude
        const newLongitude = prevLocation.coords.longitude + 0.0001; // Increment longitude

        // Update the route with the new location if endCoordinate is available
        if (endCoordinate) {
          updateRoute(
            {
              coords: {
  ...
  useEffect(() => {
    ...
    if (currentLocation && endCoordinate) {
      ...
      if (dist <= 5) {
        try {
          // Check if distance is 5 meters or less
        ...
        // setMessage(""); // Reset message if distance is greater than 5 meters
      }
    }
